...
async def test_chart_is_seeded_idempotently_and_extendable(session_factory, shop):
    async with session_factory() as s:
        ...
        assert len(chart) == 26 and all(a.is_system for a in chart.values())
        await ensure_standard_chart(s, shop)
        assert (await s.execute(select(func.count(Account.id)))).scalar_one() == 26
...
async def test_owner_endpoints(session_factory, shop):
    async with session_factory() as s:
        ...
        assert [r.code for r in rows][:3] == ["1100", "1110", "1120"] and len(rows) == 26
        ...
        assert len(await list_accounts(ctx, include_inactive=False)) == 26
        assert len(await list_accounts(ctx, include_inactive=True)) == 27
        await s.commit()
