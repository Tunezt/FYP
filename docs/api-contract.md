...
| GET | `/api/orders?q=&limit=&offset=` · `/api/orders/{id}/receipt` · POST `/api/orders/{id}/void` · `/api/orders/{id}/refund` | owner | the reversal screen (M15-T11), owner side: every sale newest first with `{number, sold_at, status, total, line_count, staff_name, customer_name}`, searchable by receipt number (`q`, the id's last 8 characters, case- and dash-insensitive); the receipt through the same shaping function the till prints; and the reversal itself, manager-PIN gated exactly as at the till (403 on a plain staff PIN, 409 on an already-reversed order), writing an `approvals` row with `channel="dashboard"` in the request log. `refund` takes `restock` |
| POST | `/api/backdated-sales` | owner | a sale that happened on paper, entered afterwards (M15-T10): `{sold_at, staff_id, lines:[{item_id, variant_id?, quantity}], payment_method, customer_id?, note?}`. Posts through the ordinary engine — same pricing, same atomic stock guard (409 on oversell), same journal entry, same points — priced server-side so the payment cannot disagree. Tagged `entry_source = 'manual_backdated'` (on every order row and in `/api/orders`) and deliberately **not** attached to any open shift, since the money was taken days ago and is not in tonight's drawer. Refused: a future date, more than 60 days old, before the business was registered, an unknown or inactive cashier (all 422) |
...
| GET | `/pos/orders?q=&limit=` | pos | today’s sales, newest first — the list the reversal screen searches (M15-T11). “Today” is the **business day** (M15-T4), so a 23:50 sale is still listed at 00:15. Open e-menu tickets are excluded: they are not sales and have their own screen. `q` matches the receipt number, case- and dash-insensitive |
| POST | `/pos/orders/{id}/void` | pos | `{manager_pin, note?}` — owner **or manager** PIN (M15-T7); reversing lines, payments and `sale_void` stock movements; original untouched; 403 wrong PIN, 409 already reversed |
