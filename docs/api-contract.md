...
| GET | `/pos/shift` · POST `/pos/shift/open` · POST `/pos/shift/close` | pos | the cashier's till session (M7-T1): open with `{opening_float}` (one open shift per cashier, 409 otherwise); read it with live `expected_cash` = float + cash payments − cash refunds attributed to the shift; close with `{counted_cash, notes?}` → expected, counted and variance written once |
...
| GET | `/api/shifts?limit=` | owner | shifts newest first with staff name, float, cash taken / handed back, expected, counted, variance |
