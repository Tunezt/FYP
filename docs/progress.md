...
### Decision record: dine-in is an open bill per table (owner, 2026-09-18)
**Status:** decided by the owner; it supersedes parts of the svc/prt sequence.
- **Owner's words, paraphrased:** paying before every order is a hassle for a table that is "vibing" and wants another drink. The cashier should add to the table's order from Pesanan aktif. Only one device (the cashier's tablet) is in use, so a separate Bar tab is unnecessary. Nobody carries the tablet to tables, so *Siap / Sudah diantar* statuses do not fit the room. An order closes when the customer pays.
- **Answers to the questions asked:** takeaway is paid at the front, as now. Driver orders come from other apps and are out of scope. No split bill (the café handles it). Removing a sent item is the cashier's responsibility: no manager PIN, but a reason. Stock is recorded when paid. For QR orders, staff bring the nota to the table as confirmation and the guest waits. Warn about open bills at shift close. Keep the kitchen screen but hide it, on the till and in the dashboard. The nota shows only the new items plus the running total. Guests may add more from their phone, and the cashier accepts.
- **Consequences:** M13 is partly revived: an open bill per table, with no split and no merge. The per-item serving states drafted as prt-5 were dropped and never landed. The till's Bar view goes (bill-3).
- **Sequence:** bill-1 backend, bill-2 QR joins the table's bill, bill-3 till UI, bill-4 docs and the end-to-end run.


### [bill-1] A table keeps one open bill: send in batches, print only what is new, pay once
**Date:** 2026-09-18
**Status:** done
**Changed:** backend/alembic/versions/0041_open_bills.py (new: `print_jobs.kind` also allows `nota`), backend/app/models/models.py (comment), backend/app/services/bills.py (new), backend/app/services/tickets.py, backend/app/services/printing.py, backend/app/api/pos.py (`POST /pos/open-orders/{id}/send`, `POST /pos/open-orders/{id}/lines/{uid}/cancel`, `send` on drafts and updates), backend/app/schemas/menu.py, backend/tests/test_open_bills.py (new, 8 tests)
**Gates:** pytest 606 passed 0 skipped - migrations round-trip ok (0041 -> 0040 -> 0041) - frontend build ok - seed ok
**Notes:**
- **Sending is the new step, not paying.** A dine-in open order with a table is the table's bill. *Kirim* stamps the unsent lines with batch n (`cart.batches`, `sent_batch` per line) and, in the same transaction, owes paper for exactly those lines: a Bar slip, a Dapur slip (only for stations that have items), and a **nota** on the front printer with the new items and their prices, "Pesanan ini", TOTAL SEMENTARA and BELUM DIBAYAR. From the second send on, slips and nota say TAMBAHAN and "Tambahan n". `send: true` on `POST /pos/drafts` and `PUT /pos/open-orders/{id}` saves and sends in one step; a failure rolls both back.
- **Paying closes it.** Settling a bill that was sent first sends anything still unsent as one last batch of slips (no nota), then runs the ordinary sale on the same row. That prints only the receipt. Stock, cost snapshots, payments and the journal are written at payment, as before. A dine-in order that was never sent, and every takeaway, prints receipt and slips at payment exactly as before.
- **Sent lines are locked.** A PUT replaces only the unsent part. A sent line the device repeats unchanged (matched by `uid`) is kept; one it repeats *changed* is refused (409); one it omits stays. Removing a sent item is `.../lines/{uid}/cancel` with a required reason and an optional quantity, and no PIN, as the owner decided. For that station, the slip is withdrawn if nothing has taken it and nothing else is on it; otherwise a BATAL notice names just those items, the quantity and the reason. The cancellation stays in `cart.voids`. Cancelling a whole bill with sent items needs a reason and tells every station the same way. An unsent bill is cancelled as before.
- **One open bill per table.** Labels are compared as people say them ("Meja 7", "meja7", " MEJA  7 " and "7" are one table). A second bill, or moving a bill onto a busy table, is a 409 naming the open bill, with its id in `X-Open-Bill-Id` so the till can open it. A per-table advisory lock makes four simultaneous "open Meja 9" taps produce one bill (tested). Takeaway is never a table.
- **Only once.** Sends are serialised on the order row (`FOR UPDATE`) and guarded by `rev`. Three simultaneous taps give one 200 and two 409s, and one set of paper. A replayed `client_ref` on "open and send" returns the same bill and prints nothing twice. Settling also takes the row lock first.
- **Stock, stated plainly.** The owner chose to record stock at payment, and the database refuses negative stock. A bill whose counted item ran out on the books before payment could not be paid for food already eaten. So a send is refused when the items, plus what other open bills have already sent, exceed the recorded stock ("Stok Roti tidak cukup untuk dikirim — tersisa 10 setelah pesanan meja lain"). Made-to-order drinks take their components at payment, as they always have. **Remaining gap:** a pay-now sale at the till can still take a counted item that an unpaid bill was promised. That bill's payment would then be refused until the stock is corrected. It only happens if the books disagree with the shelf, and it is flagged for the owner, not hidden.
- **Refunding a paid bill** tells each station about its items, batch by batch, through the same rule (tested).
- The QR menu, the till screens and the kitchen screen are untouched in this task (bill-2, bill-3).
**Deviation:** none. The `nota` kind is additive (the check is widened; the downgrade restores the narrower check as NOT VALID so no row is ever deleted). No other schema change: the batch bookkeeping lives in the unpaid order's `cart` jsonb, like the rest of it.
**Next:** bill-2. A dine-in QR order joins the table's open bill; guest-added items wait for the cashier to accept and send them.


### [bill-2] A QR guest joins the table's bill, and can order another round
**Date:** 2026-09-18
**Status:** done
**Changed:** backend/app/services/tickets.py (`place_or_join`, `add_guest_lines`, `guest_backlog`, `key_ref_for`, `guest_ref`), backend/app/services/bills.py (lines keep `guest_ref`/`guest_name`), backend/app/api/menu.py (join on placing; `POST /menu/{token}/orders/{id}/lines`; keyed view of a till-opened bill; `stage`), backend/app/api/pos.py + schemas/menu.py (`from_guest`, `guest_name` on lines; `mine`, `sent`, `stage`, `can_add` for the guest), backend/tests/test_open_bills.py (+4 tests), frontend/app/menu/[token]/page.tsx
**Gates:** pytest 610 passed 0 skipped - migrations round-trip ok (0041 -> 0040 -> 0041) - frontend build ok - seed ok
**Notes:**
- **Joining.** A dine-in guest who names a table that already has an open bill (opened at the till or by another phone) joins it. Each phone gets its own key, and its lines carry a hash of that key, never the key itself. The join is serialised on the same per-table lock as the till, so two phones at an empty table make one bill (tested). A retried tap is found by its `client_ref` and adds nothing twice. The guest's quote and `expected_total` are for their own items only; a mismatch writes nothing.
- **The cashier accepts by sending.** A guest's items arrive unsent and marked "from QR" with the guest's name. Nothing is made, printed or charged until the cashier's *Kirim*, which prints the Bar/Dapur slips and the nota that staff take to the table as confirmation (owner's answer 7). The cashier can also edit unsent guest items before sending.
- **Another round from the phone.** `POST .../lines` with the phone's key adds to the bill while it is open. Once paid it is refused ("sudah dibayar"), and the page starts a new order with the cart intact.
- **The guest is told only what is known:** *Menunggu konfirmasi kasir* (some of their items not yet sent), *Pesanan dikirim ke dapur/bar*, *Lunas*, or for takeaway *Sebutkan nomor ini di kasir* then *Sudah dibayar · sedang dibuat*. Each line says "Menunggu kasir" or "Dikirim ke dapur/bar", and "kamu" marks the phone's own items when others share the bill. The kitchen-screen steps (Masuk dapur / Disiapkan / Siap) are gone from the page; `kitchen_state` stays in the API for the hidden kitchen screen.
- **Privacy.** A phone that joined sees the table's items, the running total and the table, but not the name, phone or note of whoever started the bill. A bill opened at the till is visible only with a key (404 without). A QR ticket keeps the svc-5 keyless public view.
- **The public cap covers joins too.** The owner's direction broke an assumption behind `test_menu.py::test_the_queue_is_capped`: all its orders name "Meja 4", so they now join one bill. The test was not changed. Instead, the cap now counts every guest submission still waiting for the cashier, new tickets, joins and add-mores alike. That is a stricter guard, and it still refuses the third order in that test. A bill is also capped at 80 lines.
- **Browser run** (seeded Poernama, phone size, driven by DOM because screenshots time out while the window is hidden). The cashier opened and sent Meja 7 (Kopi Arabica, Nasi Goreng). A guest ordered a Croissant for table "7" and joined bill 001, shown as "Menunggu kasir" beside the cashier's sent items. After the till sent it, the page showed "Pesanan dikirim ke dapur/bar" on its own within one poll. *Tambah pesanan* added a Teh Tarik ("Tambahkan ke pesanan · Meja 7"), back to waiting. Payment at the till gave "Lunas · terima kasih!" and "Total dibayar Rp 94.500". The paper, in order: Bar B0, Dapur D0 and nota (Kopi + Nasi Goreng); Bar TAMBAHAN and nota TAMBAHAN (Croissant); Bar TAMBAHAN (Teh Tarik, sent at payment, no nota); receipt with all four.
- **Observed and left as is:** the nota's and the guest page's *running* total is the catalogue estimate. Promotions, vouchers and discounts are applied by the till at payment (existing svc-2 rule), so the seeded promos made the paid total Rp 94.500 against a Rp 98.000 estimate. The estimate is never lower than what is charged, and the receipt is the final word.
**Deviation:** none. No schema change.
**Next:** bill-3. The till: sent vs unsent items, *Kirim ke dapur/bar*, table conflicts, open bills only in Pesanan aktif, the Bar tab and serving statuses removed, the kitchen screen hidden, and an open-bills warning at shift close.


### [bill-3] The till runs tables: send as they order, pay when they leave
**Date:** 2026-09-18
**Status:** done
**Changed:** frontend/components/pos/OrderPanel.tsx, frontend/components/pos/ActiveOrders.tsx, frontend/components/pos/SellScreen.tsx, frontend/lib/pos.ts, frontend/lib/api.ts (`ApiError.openBillId`), frontend/app/(dashboard)/settings/page.tsx (kitchen link hidden), backend/app/api/pos.py + schemas/menu.py (print summary per send: `batch`), backend/app/main.py (CORS exposes `X-Open-Bill-Id`), backend/app/schemas/pos.py (a whole bill can be quoted: 100 lines), backend/app/services/printing.py (Bar before Dapur within a moment), backend/tests/test_open_bills.py (+1 test)
**Gates:** pytest 611 passed 0 skipped - migrations round-trip ok (0041 -> 0040 -> 0041) - frontend build ok - seed ok
**Notes:**
- **The order panel for a table.** With *Makan di sini*, the table field comes first and the main button is **Kirim ke dapur/bar · n item**. It saves and sends in one request; slips and the nota print, and the panel clears for the next customer. *Bayar sekarang* and *Simpan, belum kirim* are the quieter options. Takeaway, pickup and delivery keep *Bayar* as the main button, as before.
- **A reopened bill shows two sections.** *Sudah dikirim* is locked, labelled by send (Kiriman pertama, Tambahan 1 ...), and each item has *batalkan*: a sheet with quantity and a required reason, no PIN. *Belum dikirim* can be edited, with QR items marked "QR · name". The total is the whole bill, quoted by the server (promotions included). The service type cannot be changed once something was sent.
- **A table that already has a bill.** Typing "meja 5" for a new order when Meja 5 is open gives *Meja ini sudah punya tagihan*. **Tambahkan n item ke tagihan itu** opens that bill with what was being typed as unsent items. *Ganti nomor meja* goes back.
- **Pesanan aktif is the unpaid list.** Paid orders leave it. Filters: Semua, **Perlu dikirim**, Meja, Bawa pulang. A bill with unsent items (a guest's QR round included) shows "Pesanan QR · Perlu dikirim · 3 item", and the tab gets a dot. The detail groups items by send, lists cancelled items with reason and time, shows each send's slips and nota with their print status, and offers *Kirim ke dapur/bar* first when something is unsent, then *Bayar*, *Tambah / ubah*, *Batalkan* (reason required once anything was sent).
- **Removed from the till:** kitchen preparation badges (Baru / Disiapkan / Siap diambil), *Tandai sudah diserahkan*, the entry point for additions to a paid order (a table adds to its open bill instead; the svc-3 API remains), and the *Dapur* link. The kitchen screen and its API are unchanged but not linked from the till or the dashboard (owner's answer 9). There was no Bar tab on `main` to remove: it existed only in the dropped prt-5 draft.
- **Closing a shift with open bills** warns "Masih ada n pesanan belum dibayar: MEJA 9 ... Uangnya belum masuk laci", and the button reads *Tetap tutup shift*. It warns rather than blocks, because a bill can rightly carry over.
- **Browser run** (seeded Poernama, tablet width, then 1280x800; driven by DOM, with one screenshot of the wide takeaway panel before the hidden window stopped rendering). Covered: Meja 5 opened and sent (Kopi Arabica + Nasi Goreng); "meja 5" typed again gave the conflict sheet, and the Teh Tarik joined the bill as unsent and was sent as Tambahan 1; the sent Nasi Goreng was cancelled with "tamu ganti menu" (its unprinted Dapur slip was withdrawn, so no BATAL was needed); the bill was paid at Rp 35.000 and left Pesanan aktif, with the receipt listing only Kopi + Teh Tarik; a QR guest's round at table 9 showed as *Perlu dikirim* with a dot and was sent from the detail; the shift-close warning named MEJA 9; takeaway kept Bayar first.
